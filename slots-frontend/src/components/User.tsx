...
const UserList: React.FC = () => {
  const [view, setView] = useState<'users' | 'transactions' | 'player_statistics' | 'spins'>('users');
  ...
  const fetchPlayerStatistics = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('http://localhost:3000/api/player-statistics', {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const fetchSpins = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('http://localhost:3000/api/spins', {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="user-container">
      ...
      <div className="button-group">
        ...
        <button onClick={() => setView('player_statistics')} className={view === 'player_statistics' ? 'active' : ''}>
          Player Statistics
        </button>
        <button onClick={() => setView('spins')} className={view === 'spins' ? 'active' : ''}>
          Spins
        </button>
      </div>
              ...
              {spins.length > 0 ? (
                spins.map((spin) => (
                  <tr key={spin.spin_id}>
                    <td>{spin.spin_id}</td>
                    <td>{spin.user_id}</td>
                    <td>{spin.bet_amount.toLocaleString(undefined, { minimumFractionDigits: 0 })}</td>
                    <td>{spin.win_amount.toLocaleString(undefined, { minimumFractionDigits: 0 })}</td>
                    <td>{new Date(spin.timestamp).toLocaleDateString('th-TH', { day: 'numeric', month: 'numeric', year: 'numeric' })}</td>
                  </tr>
                ))
              ) : (
                <tr>
                  <td colSpan={5} style={{ textAlign: 'center' }}>No spin data available</td>
                </tr>
              )}
            </tbody>
          </table>
